...
//	@Test
	...
	public void testDuplicateKeyException(){
		...
		ResponseEntity<Object> responseEntity;
			...
			HttpEntity<String> firstRquest = createRequest("1", 5);
			responseEntity = hitPostService(firstRquest,String.class);
			...
			if(responseEntity.getStatusCode().is2xxSuccessful()){
				...
				Assert.assertEquals("DuplicateKeyException", error.getStatus());
				Assert.assertEquals("Already one active Ad campaign is playing for this partner id", error.getMessage());
			}
	}
	...
	public void testNoActiveCampaignException(){
		try {
			...
			Assert.assertNotNull(error);
			Assert.assertEquals("NoActiveCampaignException",error.getStatus());
		} catch(Exception e){
	...
	public Object get(String partnerId, Class class1){
		Object object = null;
		ResponseEntity<Object> result = restTemplate.getForEntity("http://localhost:8087/ad-campaign/get?partnerId=" +partnerId ,class1);
	...
	private ResponseEntity<Object> hitPostService(HttpEntity<String> entity, Class class1){
		return restTemplate.exchange("http://localhost:8087/ad-campaign/add", HttpMethod.POST, entity, class1);
	}
